...
window.renderLanguageLearning = function () {
  ...
  shell(assets.menu, `
    ...
    <div class="language-hero panel-card compact">
      <div class="section-title">Учить язык</div>
      <div class="goal-text">Нажимай на предметы и слушай их название.</div>
      <div class="language-selected">${state.lang === 'de' ? '🇩🇪 Немецкий' : '🇷🇺 Русский'}</div>
    </div>
...
window.openLearningGroup = function (id) {
  ...
  shell(assets.menu, `
    ...
    <div class="learning-header">
      ...
      <div><div class="section-title">${group.emoji} ${learningTitle(group)}</div><div class="goal-text">Нажимай на предметы и слушай их название.</div></div>
    </div>
...
window.speakLearningItem = function (groupId, index, button) {
  ...
  const utterance = new SpeechSynthesisUtterance(state.lang === 'de' ? item[1] : item[0]);
  utterance.lang = state.lang === 'de' ? 'de-DE' : 'ru-RU';
  utterance.rate = 0.78;
  utterance.pitch = 1.08;
  const voices = window.speechSynthesis.getVoices();
  const voice = voices.find(v => v.lang.toLowerCase().startsWith(state.lang === 'de' ? 'de' : 'ru'));
